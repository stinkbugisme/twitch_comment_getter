...
4. 「さらに読み込む」で追加のコメントを表示`
    ...
    },
    {
      ...
      questions: [
        {
          q: '無料プランでできることは？',
          a: `• 1日1ユーザーまで履歴表示（同じユーザーは何度でも閲覧可能）
• コメント履歴・チャンネル統計の閲覧
• ユーザー名検索・コメント検索
• お気に入りユーザー保存（最大20人）`
        },
        {
          q: '1日1ユーザーの制限とは？',
          a: '毎日リセットされる表示制限です。1日に1人のユーザーの履歴を閲覧できます。同じユーザーは何度表示しても制限にカウントされません。翌日になるとリセットされ、新しいユーザーを閲覧できるようになります。'
        }
      ]
    },
    {
...
2. Twitchアカウントでログイン
3. 「プレミアム設定」をクリック
4. 「月額¥500で登録する」ボタンをクリック
5. Stripe決済画面で支払い完了`
        },
        {
          q: '複数デバイスで使えますか？',
          a: 'Twitchアカウントに紐づくため、同じアカウントでログインすればどのデバイスでもプレミアム機能をご利用いただけます。'
        }
      ]
    },
    {
...
• そのユーザーがコメントをしていない
• チャンネルがまだ監視対象に入っていない
• ログインしていない（Twitchアカウントでのログインが必要です）
...
• 他サイトの閲覧履歴`
        ...
        {
          q: 'データはどこに保存されますか？',
          a: `• チャットメッセージ：Supabaseクラウドデータベース
• お気に入りリスト：ブラウザのローカルストレージ
• 認証情報：Supabase Auth（安全に管理）
